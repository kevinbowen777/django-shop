...
::
    ...
    * coverage-3.13 -> Check test coverage & generate HTML/JSON reports.
    ...
    * pyright-3.13 -> Type checking with pyright.
...
The ``lint``, ``safety``, and ``tests`` are enabled to be run with ``nox -s tests``. Gennerating documentation, (e.g. ``nox -s docs-3.13``) need to be run explicitly.
